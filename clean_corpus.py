...
def clean_corpus(input_path, output_path):
    ...
    This utility is particularly useful for cleaning Project Gutenberg texts
    which often contain metadata, headers, and special formatting.
    ...
    # Remove text between square brackets (often metadata/headers)
    # Handle potential nesting by repeatedly applying the regex
    prev_text = None
    ...
    # Replace fancy typeset quotes with standard ASCII quotes
    original_text = original_text.replace('"', '"').replace('"', '"')
    original_text = original_text.replace(''', "'").replace(''', "'")

    # Define allowed characters: letters, digits, whitespace, and basic punctuation
    # This keeps the vocabulary small and focused on actual content
    ...
    if removed_chars:
        print(f"Removed characters: {repr(''.join(sorted(removed_chars)))}")
