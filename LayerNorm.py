...
class LayerNorm(Module):
    ...
    Normalizes activations along the feature dimension (last axis) for each
    sample independently. Includes learnable scale (gamma) and shift (beta)
    parameters.
    """
    ...
    def forward(self, x):
        ...
        Args:
            x (array): The input data batch, typically of shape (Batch_Size, Sequence_Length, Embedding_Dim).

        Returns:
            array: The normalized output data of the exact same shape as x.
        """
        # Calculate mean and variance along the embedding dimension (last axis)
    ...
    def backward(self, grad_output):
        ...
        Args:
            grad_output (array): The gradient of the loss with respect to this layer's output,
                                 typically of shape (Batch_Size, Sequence_Length, Embedding_Dim).

        Returns:
            array: The gradient of the loss with respect to this layer's input, matching the shape of x.
        """
        ...
        if self.dgamma is None:
            ...
            self.dgamma = self.dgamma + dgamma_step
            self.dbeta = self.dbeta + dbeta_step
        # end if
        ...
        dx = (grad_x_hat - mean_grad_x_hat - self.x_hat * mean_grad_x_hat_times_x_hat) / self.std
